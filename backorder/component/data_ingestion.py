...
class DataIngestion:
    ...
    ):  # this data_ingestion_config:DataIngestionConfig parameter is necessary to intialise object of this class
        try:
    ...
    def download_backorder_data(
        ...
        try:
            ...
            os.makedirs(
                tgz_download_dir, exist_ok=True
            )  # if above folder is not available create it and if available then proceed

            backorder_file_name = os.path.basename(download_url)  # extract file name of zip file

            tgz_file_path = os.path.join(
                tgz_download_dir, backorder_file_name
            )  # complete file path
            ...
            urllib.request.urlretrieve(
                download_url, tgz_file_path
            )  # download file from  url passed and location where to download
    ...
    def extract_tgz_file(self, tgz_file_path: str):
        try:
            ...
            raw_file_path = os.path.join(
                raw_data_dir, os.path.basename(tgz_file_path).replace(".gz", "")
            )
            if os.path.exists(tgz_file_path):
