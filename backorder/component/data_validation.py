...
class DataValidation:
                ...
                if len(list(os.listdir(paths))) >= abs(number_of_n_runs_old):
                    ...
                    if os.path.exists(old_file_path):
                        ...
                        if is_data_drift_found == False:
                            logging.info(
                                f"Old and new dataset Data drift check successfull. No data drift found"
                            )
                        else:
    ...
    def is_train_test_file_exists(self) -> bool:
        try:
            ...
            if not is_available:
                ...
                message = (
                    f"Traning file : {training_file} or testing file : {test_file} is not present"
                )
    ...
    def validate_dataset_schema(self) -> bool:
        try:
            ...
            is_schema_validation_successful = (
                schema_config[SCHEMA_VALIDATION_COLUMNS_KEY] == train_schema_dataframe
                and schema_config[SCHEMA_VALIDATION_COLUMNS_KEY] == test_schema_dataframe
            )
    ...
    def save_data_drift_report(self):
        try:
            ...
            with open(report_file_path, "w") as report_file:
                json.dump(report, report_file, indent=6)
            logging.info(
                "Successful : Checking data drift train and test set and genrating json reports"
            )
            return report
        except Exception as e:
            raise BackorderException(e, exc_info()) from e
    ...
    def save_data_drift_report_old_data_check(self):
        try:
            logging.info(
                "Checking data drift for old dataset and new dataset and genrating json report"
            )
            train_df, test_df = self.get_train_test_dataframe()
            drift_report = Report(metrics=[DataDriftPreset(), TargetDriftPreset()])
            drift_report.run(reference_data=train_df, current_data=test_df)
