...
class Configuartion:
    def __init__(
        ...
        try:
            self.config_info = read_yaml_file(
                file_path=config_file_path
            )  # taken path from above, reading it using yaml fun (util)
    ...
    def get_data_ingestion_config(self) -> DataIngestionConfig:
        try:
            ...
            )  # artifact dir path taken from training_pipeline_config defined below
            ...
            ]  # raeding url from data ingestion info (which carries config.yaml info) using constant
    ...
    def get_data_transformation_config(self) -> DataTransformationConfig:
        try:
            artifact_dir = (
                self.training_pipeline_config.artifact_dir
            )  # acessing root folder of artifact
    ...
    def get_model_trainer_config(self) -> ModelTrainerConfig:
        try:
            ...
            model_trainer_artifact_dir = (
                os.path.join(  # giving path model trainer based on timestamp
                    artifact_dir, MODEL_TRAINER_ARTIFACT_DIR, self.time_stamp
                )
            )
            model_trainer_config_info = self.config_info[
                MODEL_TRAINER_CONFIG_KEY
            ]  # giving info from config.yaml using their constant
    ...
    def get_model_evaluation_config(self) -> ModelEvaluationConfig:
        try:
            ...
            ]  # reading config.yaml using its constant
            artifact_dir = os.path.join(
                self.training_pipeline_config.artifact_dir,  # creating path for artifact of evaluation
                MODEL_EVALUATION_ARTIFACT_DIR,
            )

            model_evaluation_file_path = os.path.join(
                artifact_dir,  # creating path for model evaluation inside artifact
                model_evaluation_config[MODEL_EVALUATION_FILE_NAME_KEY],
            )
            response = ModelEvaluationConfig(
                model_evaluation_file_path=model_evaluation_file_path, time_stamp=self.time_stamp
            )
    ...
    def get_model_pusher_config(
        ...
        ModelPusherConfig
    ):  # we rae creating path for saved moded (to save best model detected by data evaluation)
        try:
    ...
    def get_training_pipeline_config(self) -> TrainingPipelineConfig:
        try:
            ...
            ]  # from config info defined above reading t_p_conf_key
            artifact_dir = os.path.join(
                ROOT_DIR,
